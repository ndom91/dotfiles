...
return {
    ...
    dependencies = {
        ...
        {"williamboman/mason-lspconfig.nvim"}, {"lvimuser/lsp-inlayhints.nvim"},
        {"b0o/schemastore.nvim"}, {"jose-elias-alvarez/null-ls.nvim"}, -- Autocompletion
        ...
        {"L3MON4D3/LuaSnip"}, {"rafamadriz/friendly-snippets"}, -- Languages
        {"jose-elias-alvarez/typescript.nvim"}, {"folke/neodev.nvim"} -- lua support for nvim config + development
    },
    config = function()
        ...
        lsp.setup_nvim_cmp({
            ...
            sources = {
                {name = "copilot"}, {name = "nvim_lsp", max_item_count = 10},
                {name = "nvim_lsp_signature_help"}, {name = "luasnip"}, {name = "treesitter"},
                {name = "buffer", max_item_count = 5}, {name = "path"}, {name = "nvim_lua"}
            },
            formatting = {
        ...
        lsp.ensure_installed({
            "tsserver", "html", "jsonls", "dockerls", "bashls", "vimls", "tailwindcss", "lua_ls",
            "cssls", "volar", "yamlls"
        })
        ...
        lsp.on_attach(function(client, bufnr)
            ...
            bind("n", "<leader>lf", "<cmd>lua vim.lsp.buf.format({ timeout_ms = 2000 })<CR>",
                 {desc = "Format Buffer", buffer = bufnr})
            bind("n", "<leader>a", "<cmd>lua vim.lsp.buf.code_action()<CR>",
                 {desc = "Code [A]ction", buffer = bufnr})
            bind("n", "<leader>re", "<cmd>lua vim.lsp.buf.rename.float()<CR>",
                 {desc = "[Re]name", buffer = bufnr})
            ...
            bind("n", "<leader>D", vim.lsp.buf.type_definition,
                 {desc = "Type [D]efinition", buffer = bufnr})
            ...
            bind("n", "<C-k>", vim.lsp.buf.signature_help,
        ...
        null_ls.setup({
            ...
            sources = {
                ...
                -- Code Actions
                null_ls.builtins.code_actions.eslint_d, null_ls.builtins.code_actions.shellcheck, -- Formatting
                null_ls.builtins.formatting.eslint_d,
                null_ls.builtins.formatting.stylua,
                -- null_ls.builtins.formatting.lua_format
                --     .with({extra_args = {'--tab-width=2', '--column-limit=100'}}),
                null_ls.builtins.formatting.shfmt, -- Diagnostics
                null_ls.builtins.diagnostics.eslint_d, null_ls.builtins.diagnostics.shellcheck
            }
        })
